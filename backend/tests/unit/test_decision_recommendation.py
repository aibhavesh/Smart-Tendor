...
def _qual(qualified: bool | None) -> QualificationResult:
    """Build a screen outcome. ``None`` is the indeterminate case (rule 1b)."""
    status = {
        True: EligibilityStatus.ELIGIBLE,
        False: EligibilityStatus.NOT_ELIGIBLE,
        None: EligibilityStatus.INDETERMINATE,
    }[qualified]
    rules = [
        QualificationRuleResult(
            "technical_similar_works", qualified, "work ok", qualifying_project_name="P"
        ),
        QualificationRuleResult("financial_turnover", qualified, "turnover ok"),
        QualificationRuleResult("technical_work_type", qualified, "scope matched"),
    ]
    return QualificationResult(status=status, rules=rules)
...
def _ctx(
    *,
    ...
    return RecommendationContext(
        ...
        financials=CompanyFinancials(turnover=Decimal(turnover) if turnover else None),
...
def test_win_probability_turnover_buffer():
    # base 70 + turnover buffer 10 (301 > 3x100) = 80.0. The net-worth factor
    # is retired with FR-303 and contributes nothing.
    ctx = _ctx(sim=None, ev="100", turnover="301")
    assert _win(_qual(True), _risk("0"), ctx) == Decimal("80.0")


def test_the_buffer_compares_against_tender_value_not_the_requirement():
    # The multiplicand is V. At three times V the bonus must NOT fire on a
    # turnover that merely exceeds V, which is what 3x the requirement would
    # amount to when N is 3.
    assert _win(_qual(True), _risk("0"), _ctx(ev="100", turnover="101")) == Decimal("70.0")
    assert _win(_qual(True), _risk("0"), _ctx(ev="100", turnover="301")) == Decimal("80.0")


def test_win_probability_clamped_to_95():
    # 70 + 15 strong match + 10 turnover buffer = 95, at the clamp exactly.
    ctx = _ctx(sim="0.80", ev="100", turnover="301")
