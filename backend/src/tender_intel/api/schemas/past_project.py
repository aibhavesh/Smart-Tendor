...
class PastProjectCreateRequest(BaseModel):
    ...
    loa_reference: str | None = Field(default=None, max_length=255)
    completion_certificate_date: date | None = None
    #: Verbatim source value when the certificate column is not a date,
    #: e.g. "Running". Recorded so a blank stays distinguishable from one.
    completion_certificate_note: str | None = Field(default=None, max_length=64)
    ...
    def to_dto(self) -> PastProjectCreate:
        return PastProjectCreate(
            ...
            completion_certificate_date=self.completion_certificate_date,
            completion_certificate_note=self.completion_certificate_note,
        )
...
class PastProjectResponse(BaseModel):
    ...
    loa_reference: str | None
    completion_certificate_date: date | None
    completion_certificate_note: str | None
    ...
    def from_entity(cls, p: PastProject) -> PastProjectResponse:
        return cls(
            ...
            loa_reference=p.loa_reference,
            completion_certificate_date=p.completion_certificate_date,
            completion_certificate_note=p.completion_certificate_note,
