...
def get_work_type_repo(
    ...
    return SqlAlchemyWorkTypeRepository(session)


def get_turnover_repo(
    session: AsyncSession = Depends(get_session),
) -> SqlAlchemyCompanyTurnoverRepository:
...
def get_eligibility_repo(
    ...
    return SqlAlchemyTenderEligibilityRepository(session)


def get_portfolio_version_repo(
    session: AsyncSession = Depends(get_session),
) -> SqlAlchemyPortfolioVersionRepository:
    return SqlAlchemyPortfolioVersionRepository(session)
