...
_QUAL_CHECKLIST: dict[str, str] = {
    "technical_similar_works": "Completion certificates for the qualifying similar works",
    "financial_turnover": "CA-certified contractual turnover for the last 3 financial years",
    "technical_work_type": "Scope-of-work evidence for the matched work types",
}
...
class RecommendationEngine:
    ...
    def _verdict(
        ...
        # Rule 1 — the eligibility screen returned NOT_ELIGIBLE.
        if qualification.status is EligibilityStatus.NOT_ELIGIBLE:
            return RecommendationVerdict.NO_BID, ["rule_1_not_eligible"]
        # Rule 1b — the screen could not decide. An undecidable tender goes to
        # a person rather than being refused: grouping it with NOT_ELIGIBLE
        # would make an under-populated turnover table return NO_BID for every
        # tender at once.
        if qualification.status is EligibilityStatus.INDETERMINATE:
            return RecommendationVerdict.REVIEW, ["rule_1b_indeterminate"]
    ...
    def _win_probability(
        ...
        # The turnover buffer compares against the TENDER VALUE, never against
        # the turnover requirement. At N = 3 the requirement is V/3, so three
        # times it equals V and the bonus would fire on nearly every tender.
        ...
        if (
            ...
            and turnover > th.WIN_TURNOVER_BUFFER_MULTIPLE * ev
        ):
    ...
    def _cons(qualification: QualificationResult, risk: RiskAssessment) -> list[str]:
        cons: list[str] = []
        for rule in qualification.rules:
            # A rule that failed and a rule that could not be evaluated are
            # both watch-outs. FR-326 requires the second to be visible rather
            # than silently folded into a confidence number.
