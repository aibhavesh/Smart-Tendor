...
@dataclass(frozen=True, slots=True)
class VectorRecord:
    """A stored point with its vector, for ranking done in process."""
...
class VectorStore(Protocol):
    ...
    async def retrieve(self, collection: str, ids: list[UUID]) -> list[VectorRecord]:
        """Fetch stored vectors by id.

        The eligibility screen ranks roughly thirty past projects in process
        rather than issuing a similarity query, so it needs the vectors
        themselves. Reusing the embeddings already written on every project
        write is what keeps a second index from existing.
        """
        ...
